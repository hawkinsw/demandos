...
void configure_pci_device(void *bus_base, pci_bus_t bus, pci_dev_t dev,
  ...
  bool init_result =
      driver->init_f(driver, device_conf, bus, dev, fun, host, pci);
}
...
void load_drivers() {
  ...
  for (void *drivers_iter = drivers_start; drivers_iter < drivers_end;
       drivers_iter += sizeof(struct virtio_driver)) {
...
void pci_device_set_status(void *pci_cfg, uint8_t status) {
  ...
  volatile uint8_t *card_status_p =
      (uint8_t *)(pci_cfg + PCI_DEVICE_STATUS_OFFSET);
