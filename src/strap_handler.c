...
uint64_t epoweroff(uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e,
                   uint64_t f) {
...
uint64_t timer_interrupt_handler(void) {
#if DEBUG_LEVEL > DEBUG_TRACE
  eprint('T');
  eprint('i');
  eprint('m');
...
void configure_syscall_handlers(void) {
  ...
  syscall_handlers[93] = epoweroff; // exit
  syscall_handlers[94] = epoweroff; // exit
